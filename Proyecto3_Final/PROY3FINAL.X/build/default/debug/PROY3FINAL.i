...
void LCD_PulseEnable(void) {
    RD1 = 1; _delay((unsigned long)((1)*(8000000/4000000.0))); RD1 = 0;
}

void LCD_Cmd(unsigned char cmd) {
    RD0 = 0;
    RD2 = (cmd >> 4) & 1; RD3 = (cmd >> 5) & 1; RD4 = (cmd >> 6) & 1; RD5 = (cmd >> 7) & 1;
    LCD_PulseEnable(); _delay((unsigned long)((200)*(8000000/4000000.0)));
    RD2 = cmd & 1; RD3 = (cmd >> 1) & 1; RD4 = (cmd >> 2) & 1; RD5 = (cmd >> 3) & 1;
    LCD_PulseEnable(); _delay((unsigned long)((2)*(8000000/4000.0)));
}

void LCD_Char(unsigned char data) {
    RD0 = 1;
    RD2 = (data >> 4) & 1; RD3 = (data >> 5) & 1; RD4 = (data >> 6) & 1; RD5 = (data >> 7) & 1;
    LCD_PulseEnable();
    RD2 = data & 1; RD3 = (data >> 1) & 1; RD4 = (data >> 2) & 1; RD5 = (data >> 3) & 1;
    LCD_PulseEnable(); _delay((unsigned long)((2)*(8000000/4000.0)));
}

void LCD_Init(void) {
    TRISD = 0x00; RD0 = RD1 = 0;
...
void send_trigger(void) { RC0 = 1; _delay((unsigned long)((10)*(8000000/4000000.0))); RC0 = 0; }
unsigned int read_echo(void) {
    unsigned int timeout=0;
    while (!RC1 && timeout++<30000);
    if(timeout>=30000) return 0;
    TMR1=0; T1CON=0x10; TMR1ON=1;
    while(RC1 && TMR1<30000);
...
void ESP_SendToThingSpeak(float distance) {
    ...
    len = strlen("GET /update?api_key=") + strlen("0VKGIDGQBDYSMZEAI")
    ...
    UART_WriteString("0VKGIDGQBDYSMZEAI");
...
void main(void) {
    ...
    while(1) {
        send_trigger(); _delay((unsigned long)((100)*(8000000/4000000.0)));
        unsigned int t = read_echo();
        float d = t/58.0;
        LCD_Clear(); _delay((unsigned long)((50)*(8000000/4000.0)));
        LCD_Set_Cursor(1,1);
        if(!t) LCD_String("Sin respuesta");
        else { char b[16]; sprintf(b,"Dist:%.2fcm",d); LCD_String(b); }

        RA0=RA1=RA2=0; _delay((unsigned long)((10)*(8000000/4000000.0)));
        if(d<=10) RA0=1;
        else if(d<=30) RA1=1;
        else RA2=1;
